...
describe("لوحة الأقسام والمحرر", () => {
  ...
  it("يوفر محررًا غنيًا بعناوين وتنسيق ولون وجدول بدل حقل نص فارغ", async () => {
    ...
    expect(page).toContain('data-editor-command="formatBlock"');
    expect(page).toContain("H2 عنوان رئيسي");
    expect(page).toContain("H3 عنوان فرعي");
    ...
    expect(page).toContain("function readJson");
    expect(page).not.toContain("await response.json()");
  });
});
