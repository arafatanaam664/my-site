...
export function useAuth(options?: UseAuthOptions) {
  ...
  const logout = useCallback(async () => {
    try { await logoutMutation.mutateAsync(); }
      ...
      try { if (typeof window !== "undefined") sessionStorage.removeItem("manus-cookie"); } catch { /* storage unavailable */ }
  ...
  const state = useMemo(() => {
    if (typeof window !== "undefined") {
      try { localStorage.setItem("manus-runtime-user-info", JSON.stringify(meQuery.data)); } catch { /* storage unavailable */ }
    }
  ...
  useEffect(() => {
    ...
    if (redirectPath) { if (window.location.pathname !== redirectPath) window.location.href = redirectPath; return; }
    startLogin();
