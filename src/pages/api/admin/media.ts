...
import { discoverImageReady, optimizeImageForWeb, processingCopy } from "../../../lib/server/image-optimize";
...
export const POST: APIRoute = async ({ request }) => {
  try {
    ...
    const image = inspectSupportedImage(original);
    ...
    const optimized = await optimizeImageForWeb(original, image);
    const checksum = await sha256Hex(optimized.bytes);

    const secrets = requireMediaSecrets();
    const key = `originals/${new Date().toISOString().slice(0, 10)}/${crypto.randomUUID()}.${optimized.extension}`;
    ...
    await s3.send(new PutObjectCommand({ Bucket: secrets.R2_BUCKET_NAME, Key: key, Body: optimized.bytes, ContentType: optimized.mimeType, CacheControl: "public, max-age=31536000, immutable" }));
    const publicUrl = secrets.R2_PUBLIC_BASE_URL ? `${secrets.R2_PUBLIC_BASE_URL.replace(/\/$/, "")}/${key}` : null;
    const client = adminClient();
    const { data, error } = await client.from("media_assets").insert({ storage_key: key, public_url: publicUrl, alt_text: altText, mime_type: optimized.mimeType, width: optimized.width, height: optimized.height, bytes: optimized.bytes.byteLength, checksum_sha256: checksum, processing_status: optimized.note === "webp_optimized" ? "processed" : "original_only", processing_note: `${processingCopy(optimized)}${discoverImageReady(optimized.width, optimized.height) ? "" : " يُفضَّل لاحقًا صورة بعرض 1200 بكسل على الأقل."}`, created_by: editor.id }).select("id,storage_key,public_url,alt_text,mime_type,width,height,bytes,checksum_sha256,processing_status,created_at").single();
