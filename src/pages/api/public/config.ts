...
export const GET: APIRoute = ({ request }) => {
  try {
    return new Response(JSON.stringify({ ...publicSupabaseConfig(), authRedirectUrl: authRedirectUrl(request), developmentAccessEnabled: developmentAccessEnabled(request) }), { headers: { "content-type": "application/json; charset=utf-8", "cache-control": "no-store" } });
  } catch {
