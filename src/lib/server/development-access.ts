...
export function isDevelopmentHost(host: string) {
  ...
  return normalized === "localhost" || normalized === "127.0.0.1" || normalized.endsWith(".manus.computer");
}
...
export async function developmentAdminIdentity(request: Request) {
  const code = configFor(request); const token = cookieValue(request, cookieName);
  if (!code || !token) return null;
  const [expiresAt, signature] = token.split(".");
  if (!expiresAt || !signature || !/^\d+$/.test(expiresAt) || Number(expiresAt) * 1000 < Date.now()) return null;
...
export async function createDevelopmentSession(request: Request, suppliedCode: string) {
  ...
  return `${cookieName}=${expiresAt}.${signature}; Path=/; HttpOnly; SameSite=Strict; Max-Age=${sessionLifetimeSeconds}${secure}`;
}
