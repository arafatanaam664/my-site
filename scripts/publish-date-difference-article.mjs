...
if (content.kind !== "article" || !content.primary_media_id || !content.body_markdown || !content.seo_description) throw new Error("لا تفي المسودة بمعايير النشر: يلزم مقال ونص ووصف بحث وصورة رئيسة.");
...
const transitions = [
  { status: "in_review", note: "إحالة مقال فرق الأيام إلى المراجعة التحريرية" },
  { status: "approved", note: "اعتماد مصدرَي المقال والنص والصورة الرئيسة" },
  { status: "published", note: "نشر مقال فرق الأيام بعد اكتمال معايير الجودة" },
];
...
console.log(JSON.stringify({ status: "published", contentId: content.id, slug: content.slug, path: `/articles/${content.slug}` }));
