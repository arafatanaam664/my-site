...
const slug = "calculate-days-between-two-dates";
const localFile = "/home/ubuntu/webdev-static-assets/date-difference-article-hero.png";
const altText = "تقويم ورقي بسيط ينتقل بين نهاية شهر وبداية شهر آخر لشرح فرق الأيام";
function pngDimensions(bytes) { const signature = [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a]; if (!signature.every((value, index) => bytes[index] === value) || bytes.length < 24) return null; const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength); return { width: view.getUint32(16), height: view.getUint32(20) }; }
...
if (!dimensions || dimensions.width < 1200 || dimensions.height < 300) throw new Error("ملف الصورة غير جاهز أو لا يفي بمعيار الغلاف الرئيس.");
...
if (contentError || !content) throw contentError ?? new Error("لم يُعثر على مسودة المقال.");
...
const storageKey = `originals/${new Date().toISOString().slice(0, 10)}/date-difference-article-${checksum.slice(0, 12)}.png`;
