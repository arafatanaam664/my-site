...
const sourceFile = new URL("../docs/content-drafts/umm-al-qura-calendar-guide.md", import.meta.url);
...
if (existing) {
  console.log(JSON.stringify({ status: "already_exists", contentId: existing.id, contentStatus: existing.status }));
  process.exit(0);
}
...
const { data: content, error: contentError } = await supabase.from("content_items").insert({
  ...
  canonical_url: `https://alshafra.com/guides/${meta.slug}`,
